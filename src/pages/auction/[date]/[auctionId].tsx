...
import { ADMIN_LIST, EMPTY_ADDRESS, SPL_TOKENS } from "../../../config";
import {
  CancelOpenAuction,
  getDecimals,
  getOpenAuctionState,
  ...
  ReclaimOpenBid,
  WithdrawItemOpen,
  WithdrawWinningBidOpen,
} from "../../../contexts/transaction-auction";
...
export default function AuctionItemPage(props: {
  ...
  const getAuctionDetail = async () => {
    ...
    if (detail !== null) {
      ...
      console.log(detail.highestBid.toNumber());
      ...
      if (tokenDecimal) {
        ...
        setSplMinIncrement(
          detail.minBidTokenIncrement.toNumber() / tokenDecimal
        );
        if (detail.highestBid.toNumber() === 0) {
          setBidPrice(
            (detail.bidFloor.toNumber() + detail.minBidIncrement.toNumber()) /
              LAMPORTS_PER_SOL
          );
          setSplBidPrice(
            (detail.tokenFloor.toNumber() +
              detail.minBidTokenIncrement.toNumber()) /
              tokenDecimal
          );
        } else {
          setBidPrice(
            (detail.highestBid.toNumber() + detail.minBidIncrement.toNumber()) /
              LAMPORTS_PER_SOL
          );
          setSplBidPrice(
            (detail.highestToken.toNumber() +
              detail.minBidTokenIncrement.toNumber()) /
              tokenDecimal
          );
        }
        ...
        for (let i = 0; i < detail.bidders.length; i++) {
          ...
          if (
            detail.bidders[i].toBase58() === detail.highestBidder.toBase58() &&
            detail.bids[i].toNumber() === 0
          ) {
            setIsWithdrawed(true);
          }
          if (
            detail.bidders[i].toBase58() === wallet.publicKey.toBase58() &&
            detail.highestBidder.toBase58() !== wallet.publicKey.toBase58()
          ) {
            setIsBidder(true);
          }
        }
      ...
      const tokenName = SPL_TOKENS.find(
        (x) => x.address === tokenMint
      )?.tokenName;

      if (tokenName) setSplName(tokenName);
      setStartTime(start);
      setEndTime(end);
  ...
  const handleMakeBid = async () => {
    if (auctionId === undefined) return;
    if (bidPrice < parseInt((highestBid + minIncrement).toLocaleString())) {
      console.log(bidPrice, "====> bid price");
      errorAlertCenter(
        `The price must to be bigger than ${highestBid + minIncrement}`
      );
      return;
    }
    ...
    console.log(bidPrice, splBidPrice, "==> 1 ==> bidPrice, splBidPrice");
    try {
      await MakeOpenBid(
        wallet,
        new PublicKey(auctionId),
                ...
                <div className="create-panel">
                  ...
                  {startTime !== undefined && (
                    <>
                        ...
                        <div className="row">
                          <div className="col-half">
                            {wallet.publicKey === highestBidder ? (
                              ...
                              <button
                                className="btn-cancel-aution mt-10"
                                style={{ marginBottom: 20 }}
                      ...
                      {highestBidder === wallet.publicKey?.toBase58() &&
                        !isAdmin && (
                          <div className="row">
                            <div className="col-half">
                              <button
                                className="btn-create-aution mt-20"
                      ...
                      {ADMIN_LIST.findIndex(
                        ...
                        <div className="row">
                          <div className="col-half">
                            <button
                              className="btn-cancel-aution mt-20"
                        ...
                        <div className="auction-info-item">
                          {ADMIN_LIST.findIndex(
                            ...
                            !isWithdrawed && (
                              <button
                                className="btn-create-aution mt-20"
                                onClick={() => handleWithdrawWinningBid()}
                              >
