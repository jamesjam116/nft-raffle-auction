...
import {
  ...
  claimReward,
  getNftMetaData,
  getRaffleDataByMintAddress,
  getRaffleStateByAddress,
  revealWinner,
  withdrawNft,
} from "../../contexts/transaction-raffle";
...
export default function RaffleItem() {
  ...
  const getNFTdetail = async () => {
    setLoading(true);
    const raffleDetail = await getRaffleStateByAddress(
    ...
    if (raffleDetail) {
      ...
      const withdrawed = raffleDetail.withdrawed.toNumber();
      ...
      const solPrice =
        raffleDetail.ticketPriceSol.toNumber() / LAMPORTS_PER_SOL;
      const ticketPriceFirst =
        raffleDetail.ticketPriceFirst.toNumber() / firstDecimal;
      const ticketPriceSecond =
        raffleDetail.ticketPriceSecond.toNumber() / secondDecimal;
      ...
      if (winnerList.length !== 0) {
        const purchased = winnerList.filter(
          (item) => item.address === wallet.publicKey.toBase58()
        );
        if (purchased.length !== 0) {
          setWinner(purchased[0].address);
        }
      }
      ...
      if (solPrice !== 0 && ticketPriceFirst !== 0) {
        ...
        setPayType(
          `${ticketPriceFirst} ${firstName}  +  ${ticketPriceSecond} ${secondName}`
        );
      } else if (
        solPrice !== 0 &&
        ticketPriceFirst === 0 &&
      ...
      for (let i = 0; i < countTickets; i++) {
        if (
          raffleDetail.entrants[i].toBase58() === wallet.publicKey.toBase58()
        ) {
          mine.push({
            index: i + 1,
          });
  ...
  return (
    <>
      <Head>
        <title>Monkai | {name}</title>
      </Head>
      <div className="new-raffle">
        <div className="container">
          <h2 className="new-raffle-title">{name}</h2>
                ...
                moment(endTime).format() > moment(new Date()).format() ? (
                  ...
                  <>
                    <div className="raffle-detail">
                      ...
                      <div className="raffle-detail-item">
                        ...
                        {minePanel && (
                          <ClickAwayListener
                            onClickAway={() => setMinePanel(false)}
                          >
                            <ul>
                              {myTickets &&
                                myTickets.length !== 0 &&
                                myTickets.map((item, key) => (
                  ...
                  <>
                    <p className="no-ticket-alert">
                      Raffle is over. Not a single ticket was sold.
                    </p>
                    <div className="buy-ticket-item">
                      {isAdmin ? (
                        // withdraw NFT
                  ...
                  <>
                    <p className="no-ticket-alert">
                      Can&#39;t see a winner right now. You must pay a
                      transaction fee to view it. Do you want to continue?
                    </p>
                    <div className="buy-ticket-item">
                      <button
                        className="btn-create"
