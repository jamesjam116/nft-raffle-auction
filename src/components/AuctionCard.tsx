...
export default function AuctionCard(props: { auctionId: string }) {
    ...
    <div className="auction-card">
      ...
      <div className="media" ref={cardRef}>
        ...
        <img
          ...
          alt=""
          style={{
            filter: `${new Date() > endTime ? "grayscale(1)" : "none"}`,
      ...
      {isClosed ? (
        <div
          className="display-center flex-column auction-card-detail"
          style={{ minHeight: 150 }}
        >
          {highestBid > floorPrice ? (
            <>
              <p className="card-winning-title ">Winning Bid</p>
              <p className="card-winning">
                {highestBid}&nbsp;
                {splFloorPrice !== 0 && (
                  <span>
          ...
          <div className="auction-endtime">
            {!loading && <p className="close-time">AUCTION CLOSED</p>}
          </div>
          {highestBid > floorPrice && (
            <button
              className="btn-primary"
              onClick={() => router.push(`auction/${props.auctionId}`)}
            >
              View Winners
            </button>
          )}
        ...
        <>
          ...
          {endTime && (
            <>
              {startTime !== undefined && !loading && new Date() < startTime ? (
                <p
                  className="close-countdown"
                  style={{ padding: "10px 0", color: "#ffcb00" }}
                >
                  <CircleIcon
                    sx={{ fill: "#ffcb00", marginRight: 1 }}
                    style={{ fontSize: 12 }}
                  />
                  <span>Starts In:</span>
                  <br />
          ...
          <button
            ...
            onClick={() =>
              router.push(
                `auction/${moment(startTime).format("YYYY-MM-DD")}/${
                  props.auctionId
                }`
              )
            }
