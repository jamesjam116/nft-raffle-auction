...
import { AUCTION_API, PROGRAM_ID_AUCTION } from "../config";
...
export const CreateOpenAuction = async (
  ...
  try {
    const [auctionAddress, bump] = await PublicKey.findProgramAddress(
      [
        Buffer.from("open auction"),
        owner.toBytes(),
        Buffer.from(auctionTitle.slice(0, 32)),
      ],
      program.programId
    );

    let auctionAta = await getAssociatedTokenAccount(auctionAddress, nft_mint);
    let auctionTokenAta = await getAssociatedTokenAccount(
      auctionAddress,
      token_mint
    );
    ...
    if (!(await isExistAccount(userTokenAccount))) {
      ...
      if (userTokenAccount.toBase58() != accountOfNFT.toBase58()) {
        ...
        if (nftOwner.toBase58() == owner.toBase58())
    ...
    tx.add(
      program.instruction.createOpenAuction(
        bump,
        auctionTitle,
    ...
    await api
      .post(`${AUCTION_API}registerAuctionInfo `, {
...
export const CancelOpenAuction = async (
  ...
  try {
    ...
    let tx = new Transaction();
    tx.add(
      program.instruction.cancelOpenAuction({
        accounts: {
          auction: auctionAddress,
          owner,
          systemProgram: SystemProgram.programId,
...
export const MakeOpenBid = async (
  ...
  try {
    ...
    console.log("floor->", auctionState?.bidFloor.toNumber());
    console.log("increment->", auctionState?.minBidIncrement.toNumber());
    ...
    let tx = new Transaction();
    tx.add(
      program.instruction.makeOpenBid(
        bump,
...
export const ReclaimOpenBid = async (
  ...
  try {
    ...
    let tx = new Transaction();
    tx.add(
      program.instruction.reclaimOpenBid(bump, {
        accounts: {
          auction: auctionAddress,
          auctionAta: auctionAta,
          bidder,
...
export const WithdrawItemOpen = async (
  ...
  try {
    ...
    let tx = new Transaction();
    tx.add(
      program.instruction.withdrawItemOpen({
        accounts: {
          auction: auctionAddress,
          auctionAta: auctionAta,
...
export const WithdrawWinningBidOpen = async (
  ...
  try {
    ...
    let tx = new Transaction();
    tx.add(
      program.instruction.withdrawWinningBidOpen(bump, {
        accounts: {
          auction: auctionAddress,
          owner,
          auctionAta,
...
export const ReclaimItemOpen = async (
  ...
  try {
    ...
    let tx = new Transaction();
    tx.add(
      program.instruction.reclaimItemOpen({
        accounts: {
          auction: auctionAddress,
          auctionAta,
          owner,
...
export const getDecimals = async (
  ...
  try {
    let decimal = 1;
    await fetch(
      `https://public-api-test.solscan.io/token/meta?tokenAddress=${tokenMint.toBase58()}`
    )
      .then((resp) => resp.json())
      .then((json) => {
