...
export const getNftMetaData = async (
  ...
  let metadataAccount =
    await Metadata.getPDA(nftMintPk);
...
export const createRaffle = async (
  ...
  try {
      ...
      SystemProgram.createAccountWithSeed(
        {
          ...
          lamports:
            await solConnection.getMinimumBalanceForRentExemption(
              RAFFLE_SIZE
            ),
          space: RAFFLE_SIZE,
    ...
    if (ix0.instructions.length !== 0)
      tx.add(ix0.instructions[0]);
    if (ix1.instructions.length !== 0)
      tx.add(ix1.instructions[0]);
    console.log(endTimestamp);
    tx.add(
      program.instruction.createRaffle(
        bump,
        new anchor.BN(wlCheck),
    ...
    closeLoading();
    successAlert(
      'You succeeded in creating a Raffle!'
    );
    updatePage();
  } catch (error) {
    console.log(error);
...
export const buyTicket = async (
  ...
  try {
    let raffleState =
      await getRaffleStateByAddress(
        raffleKey
      );
    if (raffleState === null) return;

    let firstToken =
      raffleState.tokenMintFirst;
    let secondToken =
      raffleState.tokenMintSecond;

    const creator = raffleState.creator;

    let userFirstTokenAccount =
      await getAssociatedTokenAccount(
        userAddress,
        firstToken
      );
    let userSecondTokenAccount =
      await getAssociatedTokenAccount(
        userAddress,
        secondToken
      );
    let creatorFirstTokenAccount =
      await getAssociatedTokenAccount(
        creator,
        firstToken
      );
    let creatorSecondTokenAccount =
      await getAssociatedTokenAccount(
        creator,
        secondToken
      );
    let tx = new Transaction();
    tx.add(
      program.instruction.buyTickets(
        bump,
    ...
    closeLoading();
    successAlert(
      `You have purchased ${amount} tickets.`
    );
    updatePage();
  } catch (error) {
    closeLoading();
...
export const getRaffleState = async (
  ...
  let poolAccounts =
    await solConnection.getParsedProgramAccounts(
      program.programId,
      {
        filters: [
          {
  ...
  if (poolAccounts.length !== 0) {
    let rentalKey =
      poolAccounts[0].pubkey;

    try {
      let rentalState =
        await program.account.rafflePool.fetch(
          rentalKey
        );
...
export const getRaffleStateByAddress =
  async (
...
export const withdrawNft = async (
  ...
  try {
    ...
    if (ix0.instructions.length === 0) {
      tx.add(
        program.instruction.withdrawNft(
          bump,
          {
            accounts: {
              claimer: userAddress,
...
export const getRaffleGlobalState =
  async () => {
    let cloneWindow: any = window;
    let provider = new anchor.Provider(
      solConnection,
...
export const getRaffleDataByMintAddress =
